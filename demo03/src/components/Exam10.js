...
const Exam10 = () =>{
    ...
    const cancelItem = (target) =>{
        ...
        const newItems = items.map(item=>{
            ...
            if(item.itemNo === target.itemNo){//target과 같은 번호의 상품만큼은
                ...
                return{
                    ...findResult[0],    //다른 건 백업데이터로 두고
                    //필터로 안 거르고 하면 [완료] 눌렀던 다른 데이터도 백업 상태로 돌아갈까? 
    ...
    return(
        ...
        {/* <div className="row mt-4">
            <div className="col">
               상품명 <input name="itemName" value={data.itemName} onChange={changeData}/>
                가격 <input name="itemPrice" value={data.itemPrice} onChange={changeData}/>
                분류 <input name="itemType" value={data.itemType} onChange={changeData}/>
                <button type="button" className="btn btn-primary" 
        ...
        <tbody>
            ...
            {items.map((item,index)=>(
                item.edit ? (
                    ...
                    <td><input className="form-control" type="text" value={item.itemName} 
                    name="itemName" onChange={e=>changeItem(item,e)}/></td>
                    
                    <td><input className="form-control" type="text" value={item.itemPrice}
                   name="itemPrice" onChange={e=>changeItem(item,e)} /></td>
                  
                    <td><input className="form-control" type="text" value={item.itemType}
